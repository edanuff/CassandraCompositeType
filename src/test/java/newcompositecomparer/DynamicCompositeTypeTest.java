...
public class DynamicCompositeTypeTest {
	...
	private static final DynamicCompositeType comparator = new DynamicCompositeType();

	private static final int UUID_COUNT = 3;
	private static final UUID[] uuids = new UUID[UUID_COUNT];
	static {
		for (int i = 0; i < UUID_COUNT; ++i) {
			uuids[i] = UUIDGen.makeType1UUIDFromHost(FBUtilities
					.getLocalAddress());
		}
	}
	...
	public void testGetString() {
		String test1Hex = ByteBufferUtil.bytesToHex(ByteBufferUtil
				.bytes("test1"));
		ByteBuffer key = createDynamicCompositeKey("test1", uuids[1], 42, false);
		assert comparator.getString(key).equals(
				"b@" + test1Hex + ":t@" + uuids[1] + ":IntegerType@42");

		key = createDynamicCompositeKey("test1", uuids[1], -1, true);
		assert comparator.getString(key).equals(
				"b@" + test1Hex + ":t@" + uuids[1] + ":!");
	}

	// TODO: we'll need to change this when BytesType reads hex bytes with
	// fromString, which it should
	@Test
	public void testFromString() {
		ByteBuffer key = createDynamicCompositeKey("test1", uuids[1], 42, false);
		assert key.equals(comparator.fromString("b@test1:t@" + uuids[1]
				+ ":IntegerType@42"));

		key = createDynamicCompositeKey("test1", uuids[1], -1, true);
		assert key
				.equals(comparator.fromString("b@test1:t@" + uuids[1] + ":!"));
	}
